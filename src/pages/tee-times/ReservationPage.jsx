...
function ReservationPage() {
  ...
  const [rows, setRows] = useState([]); // waitlist rows
  ...
  const loading = useRef(true); // waitlist loading
  const running = useRef(false);
  const resLoading = useRef(true); // reservations loading
  const resRunning = useRef(false);

  const isMobile = useIsMobile();

  useEffect(() => {
    document.title = "Golf Tee Times";
  }, []);
  ...
  useEffect(() => {
    ...
    async function getReservations() {
      const res = await fetch(
        "https://gapps-proxy.brenthlee.workers.dev/?action=reservations",
        {
          method: "POST",
    ...
    fetch("https://gapps-proxy.brenthlee.workers.dev/?action=delete", {
      ...
      .then((data) => {
        if (data.deletedIndex === 999) {
          ...
          setToastMessage(
            `Deleted waitlist for ${popupFormData.course} on ${popupFormData.date}`,
          );
          setRows(data.data);
        }
        setToastOpen(true);
  ...
  return (
    <div className="page">
      <PageHeader name="Municipal Golf Course Tee Time Reservations/Waitlist" />
